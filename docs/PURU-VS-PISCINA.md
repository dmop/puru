...
This document is written so it can be copied into the `puru` README or benchmark docs.
...
```bash
npm run bench:piscina:node
npm run bench:piscina:bun
```
...
Takeaway: For this medium CPU workload on Node, both `puru` worker modes beat `piscina` in the sampled median. `spawn()` won this particular scenario, but that should be treated as workload-specific rather than a general rule.
...
That makes the practical positioning:
...
## Suggested Short Version For The Main README

You can use this shorter block in the main README:

> `puru` has two worker-performance stories: `spawn()` for ergonomic inline jobs, and `task()` for repeated CPU-heavy work. In local benchmarks against `piscina`, `task()` was close on heavy Fibonacci workloads and faster on one matrix workload in both Node.js and Bun, while `piscina` had lower overhead for trivial tasks. For the fairest production-style comparison, compare `puru task()` against `piscina`, not `spawn()`.
