...
These are the Go-inspired primitives built on top of the pool:

```mermaid
graph TB
    subgraph "Structured Concurrency"
        WG["WaitGroup<br/>Run N tasks, wait for all"]
        EG["ErrGroup<br/>Run N tasks, fail-fast on first error"]
    end

    subgraph "Cancellation &amp; Scoping"
        Ctx["Context<br/>Hierarchical cancel / deadline / values"]
    end

    subgraph "Communication"
        Ch["Channel<br/>Buffered send/recv between threads"]
        Sel["select()<br/>Wait for first of N promises"]
    end

    subgraph "Synchronization"
        Mu["Mutex<br/>Async mutual exclusion"]
        On["Once<br/>Execute once, share result"]
    end

    subgraph "Timing"
        Tk["Ticker<br/>Repeating interval with async iteration"]
        Af["after()<br/>Delay promise for use with select"]
    end
    ...
    Ctx -->|"signal / done()"| EG
    Ch -->|RPC bridge| Pool
    Sel -->|wraps| Promises["Promise.race + handlers"]
```

### WaitGroup & ErrGroup Flow
...
sequenceDiagram
    ...
    par All run concurrently
        P-->>WG: fn1 result
        P-->>WG: fn2 result
        P-->>WG: fn3 result
    end

    WG-->>U: [result1, result2, result3]
```

**ErrGroup** does the same, but if any task fails, it signals an `AbortController` so remaining tasks can check and bail out early.

## Putting It All Together

Here's the full picture of what happens when you run a typical puru program:

```mermaid
graph TB
    subgraph "1. Configure (optional)"
        Configure["configure({ maxThreads: 4 })"]
    end

    subgraph "2. First spawn()"
        FirstSpawn["spawn(fn)"]
        CreatePool["Create WorkerPool singleton"]
        LockConfig["Lock configuration"]
        FirstSpawn --> CreatePool --> LockConfig
    end

    subgraph "3. Task Execution"
        Serialize["Serialize function"]
        Submit["Submit to pool"]
        Route{"Exclusive\nor concurrent?"}
        ExclPath["Get/create dedicated worker"]
        ConcPath["Get/create shared worker"]
        Execute["Worker evaluates function"]
        Return["Return result via postMessage"]

        Serialize --> Submit --> Route
        Route -->|exclusive| ExclPath --> Execute
        Route -->|concurrent| ConcPath --> Execute
        Execute --> Return
    end
    ...
    subgraph "4. Cleanup"
        TaskDone["Task done"]
        NextTask{"More tasks\nqueued?"}
        Reuse["Assign next task"]
        GoIdle["Worker goes idle"]
        Timeout{"Idle\ntimeout?"}
        Terminate["Terminate worker"]

        TaskDone --> NextTask
        NextTask -->|yes| Reuse
        NextTask -->|no| GoIdle --> Timeout
        Timeout -->|yes| Terminate
        Timeout -->|no, new task| Reuse
    end

    subgraph "5. Shutdown"
        Shutdown["shutdown()"]
        Drain["Finish running tasks"]
        TermAll["Terminate all workers"]
        Shutdown --> Drain --> TermAll
    end

    Configure -.-> FirstSpawn
    Return --> TaskDone
```
